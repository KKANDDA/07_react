/*
    ...
    사용 목적은 값이 자주 계산되는 것을 방지하녀 성능 최적화..

    useCallback:
    특정 함수를 메모이제이션한다.. 함수의 재생성으 방지한다..
    자식 컴포넌트에 함수를 전달할 때 불필요하게 함수가 재생성되는 것을 방지한다..

    useCllback을 많은 하수에 남용하면 오히려 성능 저하가 발생할 수 있다..
    메모이제이션 자체도 메모리 비용이 들기 때문이다..
...
export const FunctionMemorization =()=>{
    ...
    },[]); // 넘버가 바뀔 때만 작동, 빈배열이면 최초 상태만 보여준다. 고로 0
